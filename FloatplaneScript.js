...
const API = {
    LOGIN: {
        CAPTCHA: BASE_URL + "/v3/auth/captcha/info",
        LOGIN: BASE_URL + "/v2/auth/login",
        MFA: BASE_URL + "/v2/auth/checkFor2faLogin",
        LOGOUT: BASE_URL + "/v2/auth/logout"
    },
    HOME: {
        SUBSCRIPTIONS: BASE_URL + "/v3/user/subscriptions?active=true",
        CREATOR_LIST: BASE_URL + "/v3/content/creator/list"
    },
    CREATOR: {
        INFO: BASE_URL + "/v3/creator/info",
        CONTENT: BASE_URL + "/v3/content/creator",
        PLAN: BASE_URL + "/v2/plan/info"
    },
    CONTENT: {
        DETAILS: BASE_URL + "/v3/content/post",
        RELATED: BASE_URL + "/v3/content/related",
        COMMENT: BASE_URL + "/v3/comment",
        BANNED: BASE_URL + "/v3/user/ban/status",
        MEDIA: BASE_URL + {
            VIDEO: BASE_URL + "/v3/content/video",
            AUDIO: BASE_URL + "/v3/content/audio",
            IMAGE: BASE_URL + "/v3/content/picture",
            GALLERY: BASE_URL + "/v3/content/gallery",
        },
        DELIVERY: BASE_URL + "/v3/delivery/info"
    },
    LIVE: {
        DELIVERY: BASE_URL + "/v3/delivery/info",
    },
    SELF: {
        INFO: BASE_URL + "/v3/user/self"
    }
}
...
source.getHome = function() {
    // throw new ScriptException(getToken());
    const contentData = getFloatplaneContentData();
    throw new ScriptError(JSON.stringify(contentData, null, 4));
    const query = {
...
//Internals
function getFloatplaneContentData() {
    const resp = http.GET(API.HOME.SUBSCRIPTIONS, {}, true);
    if(!resp.isOk) {
        throw new ScriptException("Failed request [" + API.HOME.SUBSCRIPTIONS + "] (" + resp.code + ")");
    }
    const contentResp = JSON.parse(resp.body);
    const creators = contentResp.map(c => c.creator);
    const query = "?" + creators.map((c, i) => "ids%5B" + i + "%5D=" + c).join("&") + "&limit=20"
    const home = http.GET(API.HOME.CREATOR_LIST + query, true);
